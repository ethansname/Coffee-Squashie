...
def main():
    ...
    pygame.display.set_caption("Coffee 'n' Squashie!")
    ...
    # get sounds
    flapfx = pygame.mixer.Sound("data/sfx/flap.wav"); flapfx.set_volume(0.5)
    upgradefx = pygame.mixer.Sound("data/sfx/upgrade.wav"); upgradefx.set_volume(0.5)
    beanfx = pygame.mixer.Sound("data/sfx/bean.wav"); beanfx.set_volume(0.5)
    deadfx = pygame.mixer.Sound("data/sfx/dead.wav"); deadfx.set_volume(0.5)
    # colors
    ...
    #splash screen
    # playing a sound
    pygame.mixer.Sound.play(flapfx); flapfx.set_volume(0.5)
    flapfx.set_volume(0.5)
    while splashScreenTimer < 100:
        ...
        startMessage = font_small.render("Don't forget to jump!", True, (171, 145, 123))
    ...
    while titleScreen:
        ...
        if (clicked and checkCollisions(mouseX, mouseY, 3, 3, DISPLAY.get_width()/2 - retry_button.get_width()/2, 288, retry_button.get_width(), retry_button.get_height())):
            clicked = False
            pygame.mixer.Sound.play(upgradefx); upgradefx.set_volume(0.5)
            titleScreen = False
        ...
        DISPLAY.blit(retry_button, (DISPLAY.get_width()/2 - retry_button.get_width()/2, 295))
        startMessage = font_small.render("START", True, (0, 0, 0))
        DISPLAY.blit(startMessage, (DISPLAY.get_width()/2 - startMessage.get_width()/2, 299))
    ...
    while True:
        ...
        if health <= 0 and not dead:
            dead = True
            pygame.mixer.Sound.play(deadfx); deadfx.set_volume(0.5)
        ...
        for bean in beans:
            ...
            if (checkCollisions(player.position.x, player.position.y, player.currentSprite.get_width(), player.currentSprite.get_height(), bean.position.x, bean.position.y, bean.sprite.get_width(), bean.sprite.get_height())):
                dead = False
                pygame.mixer.Sound.play(beanfx); beanfx.set_volume(0.5)
                beanCount += 1
                health = 100
